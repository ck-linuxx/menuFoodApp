...
export function CardButton({ food }: { food: FoodProps }) {
  return (
    <Pressable className="flex flex-col rounded-xl relative">
      ...
      <View className="flex flex-row bg-neutral-900/90 gap-1 
        rounded-full absolute top-2 right-3 px-2 py-1 items-center justify-center"
      >
      ...
      <Text className="text-neutron-600 text-sm" >{ food.time } - R${ food.delivery }</Text>
    </Pressable>
  )
}